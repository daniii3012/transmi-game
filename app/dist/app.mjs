import {NetworkMap} from './map.mjs?v=20260911.2';
import {DAY,addDays,dayType,dateNumber,dateEligible,serviceWindows,demandPeriod} from './calendar.mjs?v=20260911.2';
import {DEFAULTS,parameters} from './operation.mjs?v=20260911.2';
import {registerSimulationTools} from './webmcp.mjs?v=20260911.2';
...
try{
 const [response,contextResponse,demandResponse,layoutsResponse]=await Promise.all([fetch('./services.json',{cache:'no-store'}),fetch('./context.json',{cache:'no-store'}),fetch('./demand.json',{cache:'no-store'}),fetch('./station_layouts.json',{cache:'no-store'})]);
 if(!response.ok)throw new Error('No se pudieron cargar los servicios locales.');
 const data=await response.json();if(layoutsResponse.ok)data.station_layouts=await layoutsResponse.json();if(demandResponse.ok){data.demand=await demandResponse.json();const profiles=new Map(data.demand.profiles.map(p=>[p.station_id,p]));for(const s of data.stations){const p=profiles.get(s.id);if(p)s.demand_profile=p;}}const routeById=new Map(data.routes.map(r=>[r.id,r]));
 ...
 let viewMode=config.selection.mode,scrubbing=false,sampleSequence=0;
 ...
 let worker=new Worker('./worker.mjs?v=20260911.2',{type:'module'});
 ...
 function rebuild({fit=false,clear=true}={}){
  generation++;const messageHandler=worker.onmessage,errorHandler=worker.onerror;worker.terminate();worker=new Worker('./worker.mjs?v=20260911.2',{type:'module'});worker.onmessage=messageHandler;worker.onerror=errorHandler;ready=false;pendingSample=false;sampleSequence=0;$('#loading').hidden=false;$('#loading').textContent='Calculando despachos y estaciones…';$('#error').hidden=true;
 ...
 function syncControls(displayMode=viewMode){
  ...
  $('#peak').value=config.params.peakHeadway/60;$('#offpeak').value=config.params.offpeakHeadway/60;$('#demand').value=config.params.demand;$('#demand-value').textContent=config.params.demand+'×';$('#demand-mode').value=config.params.mode;$('#variable-dispatch').checked=config.params.variableDispatch;$('#reinforcements').checked=config.params.reinforcements;$('#cruise').value=config.params.cruiseKmh;$('#street-speed').value=config.params.streetKmh;
 ...
 $('#settings-form').onsubmit=e=>{e.preventDefault();config.params=parameters({...config.params,variableDispatch:$('#variable-dispatch').checked,reinforcements:$('#reinforcements').checked,peakHeadway:Number($('#peak').value)*60,offpeakHeadway:Number($('#offpeak').value)*60,demand:Number($('#demand').value),mode:$('#demand-mode').value,cruiseKmh:Number($('#cruise').value),streetKmh:Number($('#street-speed').value)});rebuild();toast('Escenario reconstruido con la nueva operación.');};
