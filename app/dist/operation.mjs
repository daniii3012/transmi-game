import {DAY,addDays,serviceWindows,demandPeriod} from './calendar.mjs?v=20260910.3';
import {vehicleSpec} from './vehicles.mjs?v=20260910.3';
import {travelProfile,travelAt} from './travel.mjs?v=20260910.3';
import {generatedPassengers,alightFraction} from './passengers.mjs?v=20260910.3';
import {MetricPath} from './simulation.mjs?v=20260910.3';
export const DEFAULTS=Object.freeze({peakHeadway:240,offpeakHeadway:480,demand:1,mode:'auto',cruiseKmh:48,streetKmh:30,acceleration:.8,braking:1.1,turnaround:240,capacity:160,biarticulatedShare:0,biarticulatedCapacity:250});
export function parameters(input={}){const p={...DEFAULTS,...input};for(const [k,min,max] of [['peakHeadway',120,1200],['offpeakHeadway',180,1800],['demand',.25,3],['cruiseKmh',25,60],['streetKmh',15,40],['acceleration',.4,1.4],['braking',.5,1.8],['turnaround',60,900],['capacity',80,250],['biarticulatedShare',0,100],['biarticulatedCapacity',240,250]])if(!Number.isFinite(p[k])||p[k]<min||p[k]>max)throw new Error('Parámetro fuera de rango: '+k);if(!['auto','peak','offpeak'].includes(p.mode))throw new Error('Demanda inválida');return p;}
...
export class Operation {
 constructor(data,config={}){
  ...
  for(const r of data.routes.filter(r=>r.ready&&picked(r)))this.routes.set(r.id,{...r,path:new MetricPath(r.points)});
 ...
 prepareDirections(){
  ...
  for(const r of this.routes.values())for(let i=1;i<r.visits.length-1;i++){const s=r.visits[i];if(s.kind==='street')continue;const shift=(s.wagon-(s.wagons+1)/2)*64*(s.direction===0?1:-1),bound=Math.min((r.stops[i].at_m-r.stops[i-1].at_m)/4,(r.stops[i+1].at_m-r.stops[i].at_m)/4);s.at_m+=Math.max(-bound,Math.min(bound,shift));}
 }
 build(){
  ...
  for(let day=-1;day<=0;day++){
   ...
   for(const r of this.routes.values()){
    ...
    for(let wi=0;wi<windows.length;wi++){
     ...
     while(t<end){const period=demandPeriod(t,date,this.params.mode);queue.push({type:'dispatch',time:offset+t,rid:r.id,date,departure:t});t+=period==='peak'?this.params.peakHeadway:this.params.offpeakHeadway;}
    }
   }
  }
  let peak=0,active=0;
  while(queue.length){
   ...
   if(e.type==='dispatch'){
    ...
    const trip={id:`${e.date}/${r.id}/${Math.round(e.departure)}`,routeId:r.id,vehicle,start:e.time,end:Infinity,stops:[],moves:[],passengers:0,capacity:this.vehicles[vehicle].spec.capacity};
   ...
   const key=`${s.station_id}/${s.direction}/${s.wagon}`,slots=berths.get(key)||Array(s.kind==='street'?1:2).fill(0);
   ...
   if(isLast){trip.end=close;active--;queue.push({type:'release',time:close+this.params.turnaround,station:s.station_id,vehicle:trip.vehicle});continue;}
   const next=r.visits[e.index+1],distance=next.at_m-s.at_m,isStreet=s.kind==='street'||next.kind==='street';
   const v=(isStreet?this.params.streetKmh:this.params.cruiseKmh)/3.6;
   // Stop-to-stop acceleration/braking is metric; street congestion is a slower estimated cruise profile.
   const profileKey=r.id+'/'+e.index+'/'+period;
 ...
 sampleTrip(t,time){
  ...
  return {id:t.id,vehicleId:this.vehicles[t.vehicle].id,routeId:r.id,laneId:r.id,code:r.code,pattern:r.name,color:r.color,state,s,speed,speed_kmh:speed*3.6,xy:pose.xy,angle:pose.angle,
    load,capacity:t.capacity,busType:this.vehicles[t.vehicle].spec.label,typeSource:this.vehicles[t.vehicle].spec.typeSource,length_m:this.vehicles[t.vehicle].spec.length,next_stop:next?.name||'Fin del servicio',next_station:next?.station_id,stopIndex:index,stopsServed:index+(time>=stop.close?1:0),wagon:next?.wagon||1,slot:stop.slot,
