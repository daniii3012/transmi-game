import {travelProfile,travelTimeAtDistance,travelAt} from './travel.mjs?v=20260911.10';
...
export function signalTravel(path,from,to,limit,a,b,departure,signals,cache=new Map(),key=''){
 ...
 for(let pass=0;pass<=checkpoints.length;pass++){
  ...
  let profile=cache.get(profileKey);if(!profile){profile=travelProfile(path,from,to,limit,a,b,{checkpoints:checkpoints.map(s=>s.at_m-from),stops:[...forced].map(i=>checkpoints[i].at_m-from)});cache.set(profileKey,profile);}
