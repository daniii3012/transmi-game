...
const start=performance.now(),s=new Operation(data,config),build_ms=performance.now()-start;let maxFleet=0,maxQueue=0,maxSignals=0;const t=performance.now();for(let second=4*3600;second<24*3600;second+=60){s.seek(86400+second);maxFleet=Math.max(maxFleet,s.buses.length);maxQueue=Math.max(maxQueue,s.buses.filter(b=>b.state==='queue').length);maxSignals=Math.max(maxSignals,s.buses.filter(b=>b.state==='signal').length);if(s.buses.some(b=>!Number.isFinite(b.s)||!Number.isFinite(b.speed)||b.load>b.capacity||b.load<0))throw Error('Invalid bus');}const sample_ms=(performance.now()-t)/1200;s.seek(86400+7*3600);const report={mode:config.params?'stress':'default',build_ms,sample_ms,maxFleet,maxQueue,maxSignals,profileCache:s.motionCache.size,heap_MB:process.memoryUsage().heapUsed/1e6,stats:s.stats(),limits:'CPU engine only, not FPS. Fixed route capacities 80/160/240; 2.25 demand baseline, historical entries and estimated OD. Published departures and published stretch times where the feed has them; the synthetic rule elsewhere. Corroborated busway signals with estimated phases enabled.'};console.log(JSON.stringify(report,null,2));
