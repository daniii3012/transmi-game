...
| Alimentador GTFS-Realtime oficial | Identificador, **etiqueta de flota** y placa |

La etiqueta es el único dato por vehículo que existe, y es la que el bus lleva pintada.

## La etiqueta y su cero de relleno

El `VehicleDescriptor` del alimentador trae tres campos. El proyecto leía el 1 (identificador) y el
3 (placa), y descartaba el 2, que es la etiqueta: `E0067`, `K10657`, `D0204`. El feed escribe un
cero que el vehículo no lleva —`K10657` en el alimentador es **K1657** en la calle— y las dos formas
...
Desde hoy `tools/gtfs_rt.py` guarda el campo y `capture_rt.py` lo escribe en el detalle. Cuando una
captura trae las dos formas, `classify_fleet.py` las compara y cuenta las discrepancias en su salida.
...
- **Un solo día, y sábado.** La captura sigue corriendo; con una semana se verá si algún servicio
  cambia de familia por día de la semana.
...
```
python3 tools/capture_rt.py --days 8          # sigue escribiendo el detalle, ahora con etiqueta
python3 tools/classify_fleet.py               # data/curated/fleet_types.json
.../venv/bin/python tools/build_services.py   # adjunta vehicle_profile a cada ruta
...
Captura y límites del alimentador en [captura del alimentador](CAPTURA_RT_20260912.md); efecto en la
