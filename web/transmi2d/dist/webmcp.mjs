...
export function registerSimulationTools(context, actions) {
  ...
  register({name:'read_transmi_trial',title:'Leer simulación de Transmi',description:'Devuelve escenario, reloj, flota y selección actuales de la prueba local. Los patrones son de ensayo.',
  ...
  register({name:'control_transmi_clock',title:'Controlar reloj de Transmi',description:'Pausa, reanuda o cambia la velocidad de la simulación visible sin reiniciar los buses.',
    inputSchema:{type:'object',properties:{paused:{type:'boolean'},speed:{type:'integer',enum:[1,8,32]}},additionalProperties:false},
    annotations:{readOnlyHint:false,untrustedContentHint:false},execute(input){
      if(!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).some(k=>!['paused','speed'].includes(k))||
         ('paused'in input&&typeof input.paused!=='boolean')||('speed'in input&&![1,8,32].includes(input.speed)))throw new Error('Invalid clock controls');
